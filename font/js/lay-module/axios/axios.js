layui.define(['store'],function(exports) {
		...
		([
			...
			(function(module, exports, __webpack_require__) {
				...
				function isBuffer(val) {
					return val !== null && !isUndefined(val) && val.constructor !== null && !
						...
						typeof val.constructor.isBuffer === 'function' && val.constructor.isBuffer(
							val);
				}
			...
			(function(module, exports, __webpack_require__) {
				...
				function getDefaultAdapter() {
					...
					} else if (typeof process !== 'undefined' && Object.prototype.toString.call(
				...
				module.exports = function xhrAdapter(config) {
					return new Promise(function dispatchXhrRequest(resolve, reject) {
						...
						if (config.auth) {
							...
							var password = config.auth.password ? unescape(
						...
						request.onreadystatechange = function handleLoad() {
							...
							var responseHeaders = 'getAllResponseHeaders' in
								request ? parseHeaders(request
									.getAllResponseHeaders()) : null;
							var responseData = !config.responseType || config
								.responseType === 'text' ? request.responseText :
								request.response;
							var response = {
								data: responseData,
						...
						request.onabort = function handleAbort() {
							...
							reject(createError('Request aborted', config,
						...
						request.ontimeout = function handleTimeout() {
							var timeoutErrorMessage = 'timeout of ' + config
								.timeout + 'ms exceeded';
						...
						if (utils.isStandardBrowserEnv()) {
							...
							var xsrfValue = (config.withCredentials || isURLSameOrigin(
									fullPath)) && config.xsrfCookieName ?
						...
						if ('setRequestHeader' in request) {
							utils.forEach(requestHeaders, function setRequestHeader(val,
								key) {
						...
						if (typeof config.onDownloadProgress === 'function') {
							request.addEventListener('progress', config
								.onDownloadProgress);
						}

						// Not all browsers support upload events
						if (typeof config.onUploadProgress === 'function' && request
							.upload) {
							request.upload.addEventListener('progress', config
								.onUploadProgress);
						}
			...
			(function(module, exports, __webpack_require__) {
				...
				module.exports = function settle(resolve, reject, response) {
					...
					if (!response.status || !validateStatus || validateStatus(response
						.status)) {
						resolve(response);
					} else {
						reject(createError(
	...
	axios.interceptors.response.use(function(response) {
		...
		if (response.status == 200) {
			...
			if (res.code == 200) { //后端返回的数据为的code为200时为正常数据
				return res;
			} else {
	...
	axios.defaults.baseURL="http://localhost:8888/insurance"
	exports('axios', axios);	
});
