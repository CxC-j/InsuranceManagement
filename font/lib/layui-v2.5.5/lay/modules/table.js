...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util','axios'], function(exports) {
	...
	var $ = layui.$,
		...
		TPL_MAIN = [
			...
			, '{{# if(d.data.page){ }}', '<div class="layui-table-page">',
	...
	Class.prototype.render = function() {
		...
		if (options.cols.length > 1) {
			...
			th.height(that.layHeader.height() - 1 - parseFloat(th.css('padding-top')) - parseFloat(th.css(
				'padding-bottom')));
		}
				...
				if (item2.colGroup || item2.colspan > 1) {
					...
					layui.each(options.cols[i1 + 1], function(i22, item22) {
						...
						if (item22.HAS_PARENT || (childIndex > 1 && childIndex ==
								item2.colspan)) return;

						item22.HAS_PARENT = true;
						item22.parentKey = i1 + '-' + i2;

						childIndex = childIndex + parseInt(item22.colspan > 1 ?
							item22.colspan : 1);
					});
		...
		that.eachCols(function(i3, item3) {
			...
			if (item3.width === 0) {
				that.getCssRule(options.index + '-' + item3.key, function(item) {
					item.style.width = Math.floor(autoWidth >= minWidth ? autoWidth :
						minWidth) + 'px';
				});
			}
			...
			//给设定百分比的列分配列宽
				...
				that.getCssRule(options.index + '-' + item3.key, function(item) {
					item.style.width = Math.floor((parseFloat(item3.width) / 100) *
						cntrWidth) + 'px';
				});
			}
		});
			...
			axios({
				...
				if (res[response.statusName] != response.statusCode) {
					...
					that.errorView(
						...
						('返回的数据不符合规范，正确的成功状态码应为："' + response.statusName + '": ' + response
							.statusCode)
					);
				} else {
				...
				that.setColsWidth();
				typeof options.done === 'function' && options.done(res, curr, res[response
					.countName]);
			}).catch(function(e, m) {
		...
		var that = this,
			...
			render = function() { //后续性能提升的重点
				...
				layui.each(data, function(i1, item1) {
						...
						var td = ['<td data-field="' + field + '" data-key="' + key + '" ' +
							function() { //追加各种属性
								...
								if (item3.align) attr.push('align="' + item3.align +
									'"'); //对齐方式
								if (item3.templet) attr.push('data-content="' +
									content + '"'); //自定义模板
								if (item3.toolbar) attr.push(
								'data-off="true"'); //行工具列关闭单元格事件
								...
								if (!item3.field) classNames.push(
									'layui-table-col-special'); //插入特殊列样式
					...
					trs_fixed_r.push('<tr data-index="' + i1 + '">' + tds_fixed_r.join('') +
						'</tr>');
				});
	...
	Class.prototype.renderTotal = function(data, totalRowData) {
		...
		that.eachCols(function(i3, item3) {
			...
			var content = function() {
				...
				td = ['<td data-field="' + field + '" data-key="' + options.index + '-' + item3
					.key + '" ' + function() {
						var attr = [];
	...
	Class.prototype.fullSize = function() {
		...
		//减去列头区域的高度
		bodyHeight = parseFloat(height) - (that.layHeader.outerHeight() ||
		38); //此处的数字常量是为了防止容器处在隐藏区域无法获得高度的问题，暂时只对默认尺寸的表格做支持。
	...
	Class.prototype.scrollPatch = function() {
		...
		that.layFixed.find(ELEM_BODY).css('height', layMainTable.height() >= fixHeight ? fixHeight :
		'auto');
	...
	Class.prototype.events = function() {
		...
		that.elem.on('click', 'input[name="layTableCheckbox"]+', function() { //替代元素的 click 事件
			...
			layui.event.call(checkbox[0], MOD_NAME, 'checkbox(' + filter + ')', commonMember.call(
				checkbox[0], {
					checked: checked,
					type: isAll ? 'all' : 'one'
				}));
			...
			layui.each(item1, function(i2, item2) {
				...
				if (item2.colGroup) {
					...
					layui.each(cols[i1 + 1], function(i22, item22) {
						...
						if (item22.PARENT_COL_INDEX || (childIndex > 1 &&
								childIndex == item2.colspan)) return;
				...
				if (typeof id === 'object') { //如果 id 参数直接为表头数据
					...
					table.eachCols(id, function(i3, item3) {
						if (item3.field && item3.type == 'normal' && !item3.hide) {
							...
							vals.push('"' + parseTempData(item3, content, item1,
								'text') + '"');
						}
					});
				}
